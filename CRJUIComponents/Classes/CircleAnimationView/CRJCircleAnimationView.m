...
- (void)makeConfigEffective {
    ...
    // Set shapeLayer fill color & stroke color.
    self.baseCircleLayer.path = [UIBezierPath bezierPathWithOvalInRect:self.bounds].CGPath;
...
- (void)strokeEnd:(CGFloat)value animationType:(CRJAHEasingFunction)func animated:(BOOL)animated duration:(CGFloat)duration {
    ...
    if (animated) {
        ...
        keyAnimation.values               = [CRJYXEasing calculateFrameFromValue:self.circleLayer.strokeEnd toValue:value
...
- (void)strokeStart:(CGFloat)value animationType:(CRJAHEasingFunction)func animated:(BOOL)animated duration:(CGFloat)duration {
    ...
    if (animated) {
        ...
        keyAnimation.duration             = duration;
        keyAnimation.values               = [CRJYXEasing calculateFrameFromValue:self.circleLayer.strokeStart toValue:value
                                                                         func:func frameCount:duration * 60];
...
+ (instancetype)circleViewWithFrame:(CGRect)frame lineWidth:(CGFloat)width lineColor:(UIColor *)color
    ...
    CRJCircleAnimationView *circleView = [[CRJCircleAnimationView alloc] initWithFrame:frame];
